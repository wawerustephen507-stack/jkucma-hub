...
serve(async (req) => {
  ...
  const { recipient } = await req.json().catch(() => ({ recipient: "treasurer@jkucma.co.ke" }));
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: "JKUCMA Digital Hub <finance@jkucma.co.ke>",
      to: [recipient || "treasurer@jkucma.co.ke"],
  ...
  return new Response(JSON.stringify({ success: true, revenue: totalRevenue, activeCount: totalActive }), {
